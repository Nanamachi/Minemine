package Minemine::Web::Plugin::Session;
use strict;
use warnings;
use utf8;
...
my $cipher = Crypt::CBC->new({
    key => '_Z_xoR8OZSIMrDeOcjFiTYsPiq9JV1jR',
    cipher => 'Rijndael',
});
sub _session {
    my $self = shift;
    ...
    if (!exists $self->{session}) {
        $self->{session} = HTTP::Session2::ClientStore2->new(
            env => $self->req->env,
            secret => 'iE5Xm8_tS3p5WK7kLB4HEhipqv6bqrgN',
            cipher => $cipher,
        );
    }
    return $self->{session};
...
This module manages session for Minemine.
